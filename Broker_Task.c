...
void BrokerMain(UArg arg0,UArg arg1)
{
    ...
    while(true)
    {
        ...
        {
            Mailbox_pend(broker_desc->mailbox_des[MAILBOX_FROM_CAPSENSE_TO_BROKER]->mailboxHandle, &mbox, BIOS_WAIT_FOREVER);
            Mailbox_post(broker_desc->mailbox_des[MAILBOX_FROM_BROKER_TO_UART]->mailboxHandle,&mbox,BIOS_WAIT_FOREVER);
        }
    }
}
